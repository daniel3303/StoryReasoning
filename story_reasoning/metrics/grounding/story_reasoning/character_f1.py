...
class CharacterF1(StoryReasoningGroundingMetric):
    """
    F1 Score metric specifically for character grounding in story reasoning.
    ...
    def _compute_score(
        ...
        """
        Compute mean F1 score for character detections across all samples.
        ...
        Returns:
            float: Mean character F1 score across all samples
        """
        scores = []
        ...
        for story_id in reference_detections:
            ...
            if not ref_dets and not cand_dets:
                scores.append(1.0)  # If no character mentions in either, F1 is perfect
                continue
