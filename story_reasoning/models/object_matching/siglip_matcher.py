...
class SigLipMatcher(BaseMatcher):
    """
    A class for matching objects across multiple images using SigLIP embeddings with AuraFace for person class.
    For person detections, uses AuraFace for both face detection and embedding generation.

    Args:
        model_name (str): Name of the SigLIP model to use
        ...
        require_face_match (bool): If True, person detections are only matched if a face is detected and matched.
                                 If False, falls back to SigLIP matching when no face is detected.
        min_face_resolution (float): Minimum average face dimension in pixels to consider a face good for face detection
    """

    def __init__(self, model_name: str = "google/siglip-so400m-patch14-384", device: Optional[torch.device] = None,
                 ...
                 require_face_match: bool = False, use_detection_mask: bool = True, min_face_resolution: float = 112.0,
                 create_single_matches = True):
    ...
    def _compute_embedding(
        ...
        """Compute embeddings for detection. See base class for details."""
        ...
        # Process with SigLIP - only get vision embeddings
        inputs = self.processor(images=padded_img, return_tensors="pt").to(self.device)

        with torch.no_grad():
            # Call vision model directly to get image embeddings
            ...
            image_embeds = vision_outputs[1]  # Get pooled output
            image_embeds = image_embeds / image_embeds.norm(dim=1, keepdim=True)
            embedding = image_embeds[0]

        return embedding, face_embedding
